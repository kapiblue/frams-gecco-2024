...
class ExperimentABC(ABC):
    ...
    def __init__(self, popsize, hof_size, save_only_best) -> None:
        self.population_structures = None
        self.stats = []
    ...
    @staticmethod
    def stochastic_round(value): # https://en.wikipedia.org/wiki/Rounding#Stochastic_rounding
        # For example, value==2.1 should turn in most cases to int 2, rarely to int 3
        lower = math.floor(value) # returns int
        return lower + (random.random() < (value - lower))
    ...
    def make_new_population(self, individuals, prob_mut, prob_xov, tournament_size):
        ...
        # if (self.popsize * probability) below is not integer (e.g. popsize=50, prob_xov=0.333, expected number of crossovers = 50*0.333=16.65), stochastic_round() will ensure that you will get on average the expected number of crossovers per generation (e.g. 16.65: less often 16, more often 17).
        expected_mut = self.stochastic_round(self.popsize * prob_mut) # or int(...) if you accept less mutants in some cases, see the comment above
        expected_xov = self.stochastic_round(self.popsize * prob_xov) # or int(...) if you accept less crossovers in some cases, see the comment above
        assert expected_mut + expected_xov <= self.popsize, "If probabilities of mutation (%g) and crossover (%g) added together exceed 1.0, then the population would grow every generation..." % (prob_mut, prob_xov) # can be triggered due to stochastic_round() if prob_mut+prob_xov is close to 1 and the expected number of mutants or crossovers is not integer; if this happens, adjust popsize, prob_mut and prob_xov accordingly.
    ...
    def update_stats(self, generation, all_individuals):
        ...
        print("generation %d\tn_individuals%d\tworst%g\tbest%g" % (generation, len(all_individuals), worst.rawfitness, best.rawfitness))
